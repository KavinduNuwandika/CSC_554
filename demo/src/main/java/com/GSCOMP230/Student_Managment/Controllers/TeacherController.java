...
public class TeacherController {
    ...
    public String viewClassStudents(@RequestParam("courseId") Long courseId, Model model) {
        ...
        List<StudentGradeInfo> markedStudents = enrollments.stream()
                ...
                .map(enrollment -> {
                    ...
                    String grade = calculateGrade(marks); // Only calculate grade if marks are not null
    ...
    private String calculateGrade(Integer marks) {
        if (marks == null) {
            return "No Grade";  // Or any default value to represent missing marks
        }

        // Grade calculation logic based on marks
        if (marks >= 90) {
            return "A";
        } else if (marks >= 80) {
            return "B";
